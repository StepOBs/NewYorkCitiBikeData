...
def station_form(df, date_start, date_stop):
    ...
    if start_station == "":
        df = df[df['end station name'] == stop_station]
    elif stop_station == "":
        df = df[df['start station name'] == start_station]
    else:
        df = df[(df['start station name'] == start_station) & (df['end station name'] == stop_station)]
    return df
...
def age_form(df, date_start, date_stop):
    ...
    df = df[(df['birth year'] != '') & (df['birth year'] != '\N')]
    df['birth year'] = df['birth year']

    if min_age == "":
        max_age_year = date.today().year - int(max_age)
        df = df[df['birth year'] >= int(max_age_year)]
    elif max_age == "":
        min_age_year = date.today().year - int(min_age)
        df = df[df['birth year'] <= int(min_age_year)]
    else:
        min_age_year = date.today().year - int(min_age)
        max_age_year = date.today().year - int(max_age)
        df = df[(df['birth year'] >= int(max_age_year)) & (df['birth year'] <= int(min_age_year))]
    return df
