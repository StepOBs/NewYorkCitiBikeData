...
with con:
    cur = con.cursor()
    cur.execute('DROP TABLE IF EXISTS newbikes')
    cur.execute("CREATE TABLE newbikes(tripduration, starttime, stoptime, start_station_name,"
    ...
    while not (year == 2016 and month == 2):
        print 'entered while'
        if month < 10:
            month = '0' + str(month)
        else:
            month = str(month)
        db_file = '/home/stephen/Documents/College/4th Year/4th Year Project/ReducedData/{}-{} - Citi' \
        ...
        with open(db_file, 'rb') as fin:
            ...
            to_db = [(i['tripduration'], i['starttime'], i['stoptime'], i['start station name'],
                      ...
                      i['end station latitude'], i['end station longitude'], i['usertype'], i['birth year'],
                      i['gender']) for i in dr]
            cur.executemany('INSERT INTO newbikes(tripduration, starttime, stoptime,'
